...
export function LoginForm({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <Card className="w-full max-w-md border-0 bg-white/10 backdrop-blur-xl shadow-lg">
      <CardHeader className="p-4">
        {error && <div className="mb-4 rounded-lg bg-red-500/10 p-3 text-sm text-red-500">{error}</div>}
      </CardHeader>
      <CardContent className="space-y-2 px-6 py-4 pt-2 pb-7">
        {children}
        <Form {...form}>
                ...
                <FormItem>
                  <div className="flex justify-between items-center">
                    ...
                    {(emailErrorVisible || emailErrorFading) && (
                      <p className={`text-xs text-red-400 m-0 p-0 -mt-1 ${emailErrorFading ? "fade-out" : "fade-in"}`}>
